...
from app.core import celery_app, logger, settings
...
from app.models.compliance import DailyStatistics
...
def collect_all_sources(hours: int = 24):
    ...
    try:
        ...
        logger.error("Data collection task failed", error=str(e))
        collect_all_sources.retry(exc=e)
...
def run_violation_detection(lookback_hours: int = 24):
    ...
    if result and result.get("events_detected", 0) > 0:
        classify_and_create_tickets.delay()
...
def classify_and_create_tickets():
    ...
    async def _process():
        ...
        if tickets:
            ...
            from app.core.database import get_db_context as _db
                ...
                for t in assigned:
                    if t.severity == SeverityLevel.CRITICAL.value:
                        ...
                        evts_result = await db.execute(
                            select(ComplianceEvent).where(
                                ComplianceEvent.ticket_id == t.id
                            )
                        )
...
def run_full_pipeline(hours: int = 24):
    ...
    result = collect_all_sources.s(hours) | \
        run_violation_detection.s(hours) | \
        classify_and_create_tickets.s() | \
        process_escalations.s()
